...
def master_label1540():
    ...
    # using the first two sets for training
    col0 = get_collection(0)
    col1 = get_collection(1)

    num_items = col0.count() + col1.count()
    ...
    cursor = col0.find(no_cursor_timeout=True)
    for doc in cursor:
        pid = doc['_id']
