...
def predict_tags(net):
    fname = dump_dir + 'prediction_baseline_5.tsv'
    fp = open(fname, 'w')
    num_batches = n_samples/batch_size
    for t in range(num_batches):
        net.forward()
...
def master(model_dir, viz = False):
    ...
    # model_weights = caffe_root + 'models/finetune_flickr_tag_2/finetune_flickr_tag_1_2_iter_200000.caffemodel'
...
def master_0(model_dir):
    solver = caffe.SGDSolver(osp.join(model_dir, 'solver_test.prototxt'))
    solver.net.copy_from(caffe_root + 'models/finetune_flickr_tag/finetune_flickr_tag_iter_430000.caffemodel')
    # solver.net.copy_from(caffe_root + 'models/flickr_tag/flickr_tag_train_iter_80000.caffemodel')
    ...
    #     gtlist = test_net.blobs['label'].data[image_index, ...].astype(np.int)[0,0,:]
    #     estlist = test_net.blobs['fc8_flickr'].data[image_index, ...].argsort()[::-1][:5]
    #     plt.title('GT: {} \n EST: {}'.format(classes[np.where(gtlist)], classes[estlist]))
    #     plt.axis('off')
    #     plt.show()

    print 'accuracy:{0:.4f}'.format(check_accuracy(solver.test_nets[0]))
